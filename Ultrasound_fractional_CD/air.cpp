   ...
   :calculation_2D(scheme1,Step,coordi,MF,gauss_width,AirPara,mpi_rank1,mpi_size1,mpi_yarea1,mpi_zarea1,mpi_porous1)
{
	...
	swp_nn=new double* [mpi_IMAX[mpi_iindex]];swvw_nn=new double* [mpi_IMAX[mpi_iindex]];
	for(i=0;i<mpi_IMAX[mpi_iindex];i++){
		...
		swp_nn[i]=new double[mpi_JMAX[mpi_jindex]];swvw_nn[i]=new double[mpi_JMAX[mpi_jindex]];
	}
	...
	for(i=1;i<mpi_IMAX[mpi_iindex]-1;i++){
		for(j=1;j<mpi_JMAX[mpi_jindex]-1;j++){
			...
			p_nn[i][j]=-speed_air[j]*speed_air[j]/Aav[j]*diff_t*
					...
					speed_air[j]*speed_air[j]*diff_t*coef_tao*swp_nn[i][j]+
					speed_air[j]*speed_air[j]/Aav[j]*diff_t*coef_eta*swvw_nn[i][j];
		...
		for(i=1;i<mpi_IMAX[mpi_iindex]-1;i++){
			for(j=1;j<mpi_JMAX[mpi_jindex]-1;j++){	
				...
				if(cr_uc[in][jn] == 1){
					...
					((temp_v[i+1][j]-temp_v[i][j])/(Y[i+1][j]-Y[i][j])+
					(temp_w[i][j+1]-temp_w[i][j])/(Z[i][j+1]-Z[i][j]))+
					speed_air1*speed_air1*diff_t*coef_tao1*swp_nn[i][j]+
					speed_air1*speed_air1/Aav1*diff_t*coef_eta1*swvw_nn[i][j];
				}
			}
		}
	}
